...
import './aqiqah-theme-08.css';

interface AqiqahTheme08Props {
...
export default function AqiqahTheme08({ invitation, visitor, greeting }: AqiqahTheme08Props) {
    ...
    useEffect(() => {
        if (!opened) return;
        const observer = new IntersectionObserver((entries) => entries.forEach((e) => e.isIntersecting && e.target.classList.add('aq8-visible')), {
            threshold: 0.1,
        });
        document.querySelectorAll('.aq8-anim-up').forEach((el) => observer.observe(el));
    ...
    return (
        <div className="aq8-root" data-gender={genderDataAttr}>
            {/* Google Fonts */}
            <style>{`
            ...
            <svg width="0" height="0" style={{ position: 'absolute' }} aria-hidden="true">
                <symbol id="aq8-leaf" viewBox="0 -8 34 16">
                    <path d="M0 0C8 -7 24 -8 34 0C24 8 8 7 0 0Z" fill="currentColor" />
                </symbol>
                <symbol id="aq8-br" viewBox="-10 -20 250 160">
                ...
                <symbol id="aq8-br2" viewBox="-10 -20 250 160">
                    <path d="M0 0 C60 10 120 40 220 120" fill="none" stroke="#4d6b3f" strokeWidth="2.5" strokeLinecap="round" />
                ...
                <symbol id="aq8-sprig" viewBox="0 0 120 30">
            ...
            {/* ── Opening Overlay ────────────────────────────────────────────────── */}
            {coverEnabled && (
                <div className={`aq8-overlay ${opened ? 'aq8-hide' : ''}`}>
                    <div className="aq8-overlay-decor" aria-hidden="true">
                        <svg className="aq8-overlay-branch aq8-overlay-branch-l aq8-sway" aria-hidden="true">
                            <use href="#aq8-br" />
                        </svg>
                        <svg className="aq8-overlay-branch aq8-overlay-branch-r aq8-sway" aria-hidden="true">
                            <use href="#aq8-br2" />
                        </svg>
                        <span className="aq8-overlay-leaf aq8-overlay-leaf-1">🍃</span>
                        <span className="aq8-overlay-leaf aq8-overlay-leaf-2">🌿</span>
                    </div>
                    <div className="aq8-overlay-frame">
                        <p className="aq8-overlay-bism" lang="ar" dir="rtl">
                            بِسْمِ اللَّهِ الرَّحْمَنِ الرَّحِيْمِ
                        </p>
                        <p className="aq8-overlay-label">Walimatul Aqiqah</p>
                        <div className="aq8-overlay-divider" />
                        <div className="aq8-overlay-name">{invitation.babyName}</div>
                        {babyGender && (
                            <p className="aq8-overlay-gender">
                                {genderIcon(babyGender)} {babyGender}
                            </p>
                        )}
                        <div className="aq8-overlay-divider" />
                        <p className="aq8-overlay-date">{invitation.mainDateFormatted}</p>

                        {greetingEnabled && coverGuestName && (
                            <>
                                <p className="aq8-overlay-greeting-title">{greeting?.title ?? 'Kepada Yth.'}</p>
                                <p className="aq8-overlay-guest-name">{coverGuestName}</p>
                                {guestName && greeting?.guestLabel && <p className="aq8-overlay-guest-label">{greeting.guestLabel}</p>}
                            </>
                        )}
                        {greetingEnabled && greeting?.message && <p className="aq8-overlay-message">{greeting.message}</p>}
                        {invitation.guestQrData && (
                            <div className="aq8-overlay-qr">
                                <GuestQrCode
                                    data={invitation.guestQrData}
                                    size={120}
                                    style={{ borderRadius: '12px', border: '3px solid rgba(79,122,74,0.4)' }}
                                />
                                <p className="aq8-overlay-qr-label">QR Check-in Tamu</p>
                            </div>
                        )}
                        <button className="aq8-btn-open" onClick={openInvitation}>
            ...
            {/* ── Main Content ────────────────────────────────────────────────────── */}
            <div ref={mainRef} className={`aq8-main ${opened ? 'aq8-main-visible' : ''}`}>
                {/* ── HERO ──────────────────────────────────────────────────────── */}
                <section className="aq8-hero" id="top">
                    <svg className="aq8-branch aq8-br-tl aq8-sway" aria-hidden="true">
                        <use href="#aq8-br" />
                    </svg>
                    <svg className="aq8-branch aq8-br-tr aq8-sway" aria-hidden="true">
                        <use href="#aq8-br2" />
                    </svg>
                    <div className="aq8-fall" aria-hidden="true">
                    ...
                    <div className="aq8-hero-frame aq8-anim-up">
                        <p className="aq8-bism" lang="ar" dir="rtl">
                            بِسْمِ اللَّهِ الرَّحْمَنِ الرَّحِيْمِ
                        </p>
                        <span className="aq8-label">Tasyakuran Aqiqah</span>
                        <div className="aq8-wreath">
                        ...
                        <p className="aq8-hand">tumbuh bersama doa &amp; cinta</p>
                        <h1 className="aq8-hero-name">{invitation.babyName}</h1>
                        {nasabLine && <p className="aq8-hero-nasab">{nasabLine}</p>}
                        {babyGender && (
                            <div className="aq8-gender-badge">
                                <span>{genderIcon(babyGender)}</span> {babyGender}
                            </div>
                        )}
                        {invitation.birthDateFormatted && <p className="aq8-hero-sub">Lahir pada {invitation.birthDateFormatted}</p>}
                        <div className="aq8-when-line">
                        ...
                        {isEnabled('countdown') && (
                            <Countdown
                                ...
                                className="aq8-countdown"
                                boxClassName="aq8-countdown-box"
                                numClassName="aq8-countdown-num"
                                labelClassName="aq8-countdown-label"
                            />
                        )}
                        <div className="aq8-scroll-indicator">↓</div>
                    </div>
                </section>

                {/* ── SALAM / OPENING QUOTE ────────────────────────────────────────── */}
                <section className="aq8-quote" id="salam">
                    <div className="aq8-quote-inner">
                        <p className="aq8-hand aq8-anim-up">Assalamu’alaikum Warahmatullahi Wabarakatuh</p>
                        <p className="aq8-arabic aq8-anim-up" lang="ar" dir="rtl">
                            كُلُّ غُلَامٍ رَهِينَةٌ بِعَقِيقَتِهِ، تُذْبَحُ عَنْهُ يَوْمَ سَابِعِهِ، وَيُحْلَقُ، وَيُسَمَّى
                        </p>
                        <blockquote className="aq8-quote-text aq8-anim-up">
                            “Setiap anak tergadai dengan aqiqahnya; disembelihkan (hewan) untuknya pada hari ketujuh, dicukur rambutnya, dan diberi
                            nama.”
                        </blockquote>
                        <p className="aq8-quote-src aq8-anim-up">HR. Abu Dawud, dari Samurah bin Jundub</p>
                        <svg className="aq8-sprig aq8-anim-up" aria-hidden="true">
                            <use href="#aq8-sprig" />
                        </svg>
                        <p className="aq8-quote-lead aq8-anim-up">
                ...
                {isEnabled('couple_profile') && (
                    <section className="aq8-section aq8-profile" id="anak">
                        <p className="aq8-hand aq8-anim-up" style={{ textAlign: 'center' }}>
                            si kecil
                        </p>
                        <h2 className="aq8-section-title aq8-anim-up">Tunas kecil kami yang baru tumbuh</h2>
                        <div className="aq8-child-grid aq8-anim-up">
                            <div className="aq8-leafpic">
                                <div
                                    className="aq8-leafpic-photo"
                                    style={
                                        babyPhoto
                            ...
                            <div>
                                <p className="aq8-child-name">{invitation.babyName}</p>
                                {nasabLine && <p className="aq8-child-nasab">{nasabLine}</p>}
                                <p className="aq8-child-note">semoga tumbuh kuat seperti pohon yang akarnya teguh 🌱</p>
                            </div>
                        </div>
                        {parentsLine && (
                            <div className="aq8-parents aq8-anim-up">
                                <p className="aq8-parents-cap">{parentsLabel}</p>
                                <p className="aq8-parents-who">{parentsLine}</p>
                            </div>
                        )}
                    </section>
                )}
                ...
                {isEnabled('event_detail') && invitation.events.length > 0 && (
                    <section className="aq8-section aq8-events-bg" id="acara">
                        <p className="aq8-hand aq8-light aq8-anim-up" style={{ textAlign: 'center' }}>
                            waktu &amp; tempat
                        </p>
                        <h2 className="aq8-section-title aq8-light aq8-anim-up">Insya Allah diselenggarakan pada</h2>
                        <div className="aq8-events-grid">
                            {invitation.events.map((ev, i) => {
                                ...
                                return (
                                    <div key={i} className="aq8-board aq8-anim-up">
                                        <svg className="aq8-board-branch" aria-hidden="true">
                                            <use href="#aq8-br" />
                                        </svg>
                                        <h3 className="aq8-board-name">{ev.name}</h3>
                                        {parts && (
                                            <div className="aq8-dayrow">
                                                <span className="aq8-dayrow-d">{parts.weekday.toUpperCase()}</span>
                                                <span className="aq8-dayrow-n">{parts.day}</span>
                                                <span className="aq8-dayrow-m">
                                                    {parts.month}
                                                    <span>{parts.year}</span>
                                                </span>
                                            </div>
                                        )}
                                        {timeStr && <span className="aq8-board-time">Pukul {timeStr}</span>}
                                        {(ev.locationName || ev.location) && (
                                            <p className="aq8-board-addr">
                                                {ev.locationName && <strong>{ev.locationName}</strong>}
                                                {ev.location && <span>{ev.location}</span>}
                                            </p>
                                        )}
                                        <div className="aq8-actions">
                                            {mapsUrl && (
                                                <a className="aq8-btn aq8-btn-solid" href={mapsUrl} target="_blank" rel="noreferrer">
                                                    Buka Peta
                                                </a>
                                            )}
                                            <button className="aq8-btn" type="button" onClick={() => addToCalendar(ev, invitation.babyName)}>
                                                Simpan Tanggal
                                            </button>
                                        </div>
                                    </div>
                ...
                {isEnabled('location') &&
                    ...
                    (() => {
                        ...
                        return (
                            <section className="aq8-section">
                                <h2 className="aq8-section-title aq8-anim-up">Lokasi Acara</h2>
                                <div className="aq8-divider aq8-anim-up">
                                    <svg className="aq8-sprig" aria-hidden="true">
                                        <use href="#aq8-sprig" />
                                    </svg>
                                </div>
                                {ev.locationName && (
                                    <p className="aq8-location-sub aq8-anim-up">
                                        {ev.name} · {ev.locationName}
                                    </p>
                                )}
                                <div className="aq8-map-container aq8-anim-up">
                                    {ev.mapsEmbed && (
                                        <div className="aq8-map-wrapper">
                                            <iframe
                                                src={ev.mapsEmbed}
                                                width="100%"
                                                height="400"
                                    ...
                                    {mapsUrl && (
                                        <div style={{ textAlign: 'center', marginTop: '20px' }}>
                                            <a href={mapsUrl} target="_blank" rel="noreferrer" className="aq8-btn-maps">
                                                🗺️ Buka Google Maps
                                            </a>
                                        </div>
                                    )}
                ...
                {isEnabled('gallery') && invitation.gallery?.length > 0 && (
                    <section className="aq8-section aq8-gallery-bg">
                        <h2 className="aq8-section-title aq8-anim-up">Galeri Foto</h2>
                        <div className="aq8-divider aq8-anim-up">
                            <svg className="aq8-sprig" aria-hidden="true">
                                <use href="#aq8-sprig" />
                            </svg>
                        ...
                        <GallerySection
                            ...
                            styles={{
                                grid: 'aq8-gallery-grid',
                                item: 'aq8-gallery-item',
                                thumb: 'aq8-gallery-thumb',
                                overlay: 'aq8-gallery-overlay',
                                filterBar: 'aq8-gallery-filter-bar',
                                filterBtn: 'aq8-filter-btn',
                                filterBtnActive: 'aq8-filter-btn aq8-filter-btn-active',
                            }}
                        />
                    </section>
                )}
                ...
                {isEnabled('video') && babyVideoEmbedUrl && (
                    <section className="aq8-section">
                        <h2 className="aq8-section-title aq8-anim-up">Video Kenangan</h2>
                        <div className="aq8-divider aq8-anim-up">
                            <svg className="aq8-sprig" aria-hidden="true">
                                <use href="#aq8-sprig" />
                            </svg>
                        </div>
                        <div className="aq8-video-frame aq8-anim-up">
                            <iframe
                ...
                {isEnabled('digital_envelope') && (invitation.bankAccounts?.length > 0 || invitation.digitalWallets?.length > 0) && (
                    <section className="aq8-section aq8-gift-bg">
                        <h2 className="aq8-section-title aq8-light aq8-anim-up">Amplop Digital</h2>
                        <div className="aq8-divider aq8-light aq8-anim-up">
                            <svg className="aq8-sprig" aria-hidden="true">
                                <use href="#aq8-sprig" />
                            </svg>
                        </div>
                        <p className="aq8-gift-subtitle aq8-anim-up">
                        ...
                        <DigitalWalletSection
                            ...
                            onToast={showToast}
                            styles={{
                                bankGrid: 'aq8-bank-grid',
                                bankCard: 'aq8-bank-card',
                                bankLogo: 'aq8-bank-logo',
                                bankType: 'aq8-bank-type',
                                bankNumber: 'aq8-bank-number',
                                bankName: 'aq8-bank-name',
                                copyBankBtn: 'aq8-btn-copy-bank',
                                ewalletGrid: 'aq8-ewallet-grid',
                                ewalletCard: 'aq8-ewallet-card',
                                ewalletName: 'aq8-ewallet-name',
                                ewalletPhone: 'aq8-ewallet-phone',
                                copyEwalletBtn: 'aq8-btn-copy-ewallet',
                                ewalletTitle: 'aq8-ewallet-title',
                            }}
                        />
                    </section>
                )}
                ...
                {/* ── RSVP ──────────────────────────────────────────────────────── */}
                {isEnabled('rsvp') && (
                    <section className="aq8-section" id="rsvp">
                        <p className="aq8-hand aq8-anim-up" style={{ textAlign: 'center' }}>
                            RSVP &amp; doa
                        </p>
                        <h2 className="aq8-section-title aq8-anim-up">Titipkan doa untuk si kecil</h2>
                        <div className="aq8-divider aq8-anim-up">
                            <svg className="aq8-sprig" aria-hidden="true">
                                <use href="#aq8-sprig" />
                            </svg>
                        ...
                        <RSVPForm
                            ...
                            styles={{
                                form: 'aq8-rsvp-form',
                                label: 'aq8-rsvp-label',
                                input: 'aq8-rsvp-input',
                                select: 'aq8-rsvp-select',
                                textarea: 'aq8-rsvp-textarea',
                                radioGroup: 'aq8-rsvp-radio-group',
                                radioLabel: 'aq8-rsvp-radio-label',
                                errorText: 'aq8-rsvp-error',
                                submitBtn: 'aq8-rsvp-submit',
                                successBox: 'aq8-rsvp-success',
                            }}
                        />
                    </section>
                )}
                ...
                {/* ── WISHES ────────────────────────────────────────────────────── */}
                {isEnabled('wishes') && (
                    <section className="aq8-section aq8-wishes-bg">
                        <h2 className="aq8-section-title aq8-anim-up">Ucapan &amp; Doa</h2>
                        <div className="aq8-divider aq8-anim-up">
                            <svg className="aq8-sprig" aria-hidden="true">
                                <use href="#aq8-sprig" />
                            </svg>
                        ...
                        <WishesSection
                            ...
                            onToast={showToast}
                            styles={{
                                container: 'aq8-wishes-layout',
                                formBox: 'aq8-wishes-form',
                                formTitle: 'aq8-wishes-form-title',
                                nameInput: 'aq8-wish-input',
                                messageInput: 'aq8-wish-input',
                                submitBtn: 'aq8-wish-btn',
                                wishCard: 'aq8-wish-card',
                                wishAvatar: 'aq8-wish-avatar',
                                wishName: 'aq8-wish-name',
                                wishDate: 'aq8-wish-date',
                                wishMessage: 'aq8-wish-message',
                                loadMoreBtn: 'aq8-btn-more',
                            }}
                        />
                    </section>
                )}
                ...
                {/* ── CLOSING ───────────────────────────────────────────────────── */}
                {isEnabled('footer') && (
                    <section className="aq8-closing">
                        <div className="aq8-closing-decor" aria-hidden="true">
                            <span className="aq8-closing-leaf aq8-closing-leaf-1">🍃</span>
                            <span className="aq8-closing-leaf aq8-closing-leaf-2">🌿</span>
                        </div>
                        <div className="aq8-closing-frame aq8-anim-up">
                            <svg className="aq8-sprig" aria-hidden="true">
                                <use href="#aq8-sprig" />
                            </svg>
                            <p className="aq8-closing-lead">
                                Merupakan suatu kehormatan dan kebahagiaan bagi kami apabila Bapak/Ibu/Saudara/i berkenan hadir dan memberikan doa
                                restu.
                            </p>
                            <p className="aq8-closing-salam">Wassalamu’alaikum Warahmatullahi Wabarakatuh</p>
                            <p className="aq8-hand aq8-closing-from">kami yang berbahagia</p>
                            {parentsLine && <p className="aq8-closing-family">Kel. {parentsLine}</p>}
                            <p className="aq8-closing-credit">Created with love ✦ Undesia Digital Invitation</p>
                        </div>
                    </section>
                )}
            </div>
            ...
            {isEnabled('music') && invitation.music?.url && (
                <MusicPlayer
                    ...
                    triggerPlay={opened}
                    buttonStyle={{ background: 'var(--aq8-leaf)', border: '2px solid #fff', color: '#fff' }}
                />
            )}

            {/* ── Toast ─────────────────────────────────────────────────────────── */}
            <Toast message={toast} onDone={clearToast} className="aq8-toast" />

            {/* ── Back to Top ───────────────────────────────────────────────────── */}
            {showBackTop && (
                <button className="aq8-back-top" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
                    ↑
                </button>
            )}
        </div>
